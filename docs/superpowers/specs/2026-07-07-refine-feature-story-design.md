...
**Date:** 2026-07-07 · **Amended:** 2026-07-14 (design-review findings + store amendments — see
`2026-07-14-refine-feature-design-review.md`)
...
Seams: both read/write the backlog store (Spec A, as amended: the markdown tree is always the
record; an attached Miro board is a one-way mirror). refine-feature curates the story
...
(concrete past fact over speculation). Tracker-agnostic; Miro is only ever an optional
mirror of the store, never a requirement.
...
and the jargon-translation glossary. Both guides render these blocks from this spec and the brief
(the brief's §4 stop-list is **canonical** for the battery — §7 below abridges it), and the repo
README's keep-in-sync note extends to them — same discipline as the duplicated store reference.

**Frontmatter descriptions** (drafted here so the build doesn't improvise them — trigger-focused,
deliberately unlike the suite's earlier workflow-summary style, which skill-authoring guidance warns
causes agents to follow the description instead of the body):

- refine-feature: *"Use when a skeleton feature from decompose-epic needs to become build-ready
  before the team commits to it — deep on the feature, deliberately shallow on its story set —
  through a conversational SME interview. Tracker-agnostic; reads and writes the suite's backlog
  store. Use before refine-story elaborates individual stories."*
- refine-story: *"Use when one story Card from the backlog store needs its Conversation and
  Confirmation — from one-line skeleton to ready-for-sprint-planning — through a conversational SME
  interview. Use after decompose-epic (and ideally refine-feature) has shaped the feature's story
  set."*

**Session intake (both skills):** locate the backlog per Spec A §5 (directory path; the files are
the record, and an attached board mirror is re-rendered after write-back — Miro is never mentioned
when its tools are absent); list candidate items by ID and title; the user picks exactly one.
Default candidates are `skeleton`/`needs-discovery` features and `skeleton` stories; `refined`/`ready`
items open as a **revisit session** ("what do you want to change, and why?" — work only that thread,
per suite convention), and refine-feature may select a `parked` story to un-park. Verify the
upstream chain on load (hypothesis slots present, outcome link intact for a
...
shallow hypothesis-only pass — it writes back a sharpened `## Hypothesis` only, the feature **stays
`status: skeleton`**, and the knowledge-state note records the shallow pass and what was deferred.
...
A `needs-discovery` feature **re-enters here**: re-run the triage seeded by the discovery results the
team brings back — evidence upgraded → proceed on the surviving route; assumption refuted → park or
reshape the feature with the SME. The gate and the ending bars are **route-scoped** (see §9).
...
what share of the epic outcome gap does this feature close? — asked as a sanity probe that shapes
parking judgment; the answer is **never recorded as a score** (suite no-scores lock).
...
- **ADD:** journey map-walk ("first they…, then they… — have I missed a step?" — Patton); outputs
  ...
  scope (North); lifecycle and non-obvious-role sweep (Wake). Added stories are minted the next free
  `S##`, tagged `illustrative` (story front-matter `tags`), and default `kind: variation` —
  `discovery` when they exist to test the riskiest assumption.
- **SPLIT (coarse):** triggers — "manage/control" verbs, variation-hiding terms, whole narrated
  workflows, kite-level scope (Cockburn sea-level lint). Lawrence & Green patterns; outputs are new
  one-line Cards with fresh `S##` IDs (default `kind: variation`), the split parent kept as
  `status: superseded` ("Split into …"); no AC, no sizing.
...
- **REPAIR:** pseudo-stories (tasks/components in story syntax) are recombined into one good large
  ...
  Recombined-away fragments keep their files as `status: superseded`, pointing at the new Card.
...
(Adzic; Lawrence & Green). The nominee takes `order: 1` and is marked in the feature's `## Stories`
roll-up.
...
the skill flags possible hidden waste. Repair breaks in dialog before writing back. **Route scoping:**
the full pass runs (a)–(e); the fast lane runs (b)–(d) over its quantified signal and 1–2 AC, its set
*glance* flagging waste rather than curating; the discovery-brief and hypothesis-only exits skip the
gate — their bars are in §9.

**Store write-back:** the feature file updated in place — `status: refined` (or `needs-discovery`),
...
set changes (new `S##` skeleton files, `parked`/`superseded` statuses with reasons, `order` updates,
first-slice mark); the feature's `## Stories` roll-up regenerated and the epic's `## Features` row
refreshed; touched Cards re-rendered on the board mirror when one is attached.
...
- **Complexity score (Keogh), asked plainly** ("has anyone on your team / in your company / anywhere
  done this before?"). **Calibrator only — there is no spike exit** (user decision): 1–2 → **fast
  pass** (value anchor + happy-path confirm + a rule-line checklist Confirmation; the mining loop and
  sweeps are skipped, and the knowledge-state report names that they were); 3 → full conversation;
  4–5 → the Confirmation scopes to what IS known (clear hypothesis +
...
- **S:** intensity dial first (same headline, defer elaborate bullets); size probes without estimates
  ...
  North). Capacity-bet answers are elicitation only — never persisted to the store.
...
hard-blocked. Peeled Cards are minted fresh `S##` IDs (default `kind: variation`), inherit the
parent's benefit clause, and route back to the feature's story set — a peel *narrows* the parent
Card, it never supersedes it.
...
mean?" — hesitation is a defect in the artifact, not the SME (Dinwiddie); (d) **vital filter** — every
criterion reject-worthy; 3–5 is the norm (Cohn/Pichler), fewer is legitimate on a fast pass or a
4–5-complexity first-probe Confirmation; (e) **ignorance question** — "what are we most likely to
be wrong about?" → Known-Unknowns block with owners (North).

**Store write-back:** story file updated in place — `status: ready`, the Card line unchanged,
...
set with fresh `S##` IDs; the feature's `## Stories` roll-up regenerated; touched Cards re-rendered
on the board mirror when one is attached.
...
lumped requirements — Wiegers); the **"call me when you're done" test** (Wiegers). Scoping: the
brief's §4 stop-list is canonical (the list above abridges it); the battery runs over **drafted
criteria only** — never over Scale/Meter fields or the scale library, whose items legitimately
contain stop-words ("support contacts/week") — and *and/or* means the literal token (bare
conjunctions belong to the fused-AC check).
...
**Three counts, three meanings** (render crisply in both guides — they are not interchangeable):
**criteria** = top-level Confirmation lines (vital filter; 3–5 the norm) · **examples** = rows under
one rule (3–7, with boundary + counter-example) · **scenarios** = GWT scenarios (~5–6 → propose a
split).
...
upstream chain is missing (route back to the upstream skill) · reuse or renumber an item ID (IDs are
permanent, even parked/superseded) · leave a parent roll-up stale after a write-back.

## 9. Ending criteria (route-scoped — amended 2026-07-14)

The bars below are **per route**; rendering them as one unconditional list would force the skill to
either refuse legitimate fast exits or pad — the exact failure §8 prohibits.

**refine-feature — every route:** triage route chosen and named · store write-back complete (status
per route) · parent roll-ups refreshed (feature `## Stories`; epic `## Features` row) · board mirror
...
- **Full pass adds:** hypothesis in behavior-change form with evidence provenance · success signal
  with Scale, Meter, Baseline, Target, Fail level (no bare direction words) · every feature AC
  passing the dual test · NFRs bound within the critical-few budget · story set curated
  (ADD/SPLIT/PARK/REPAIR each considered; all Cards one-line; first slice nominated by the triple
  test) · dependencies/risks/deferrals captured with the Real Options ledger · Benefit-Trace Gate
  passed both ways · exits `status: refined`.
...
- **Hypothesis-only pass (timing guard) adds:** sharpened hypothesis with provenance · stays
  `status: skeleton` · the knowledge-state note records the shallow pass and what was deferred.

**refine-story — every route:** entry checks passed (narrative slots, complexity calibration) ·
value anchor confirmed (so-that + concrete output) · Confirmation drafted via the routing rules,
fully linted, every line SME-traced, scope-out line present · Demo-and-Read-Back Gate passed (vital
filter calibrated to the route) · store write-back complete (`status: ready`, Card unchanged) ·
feature `## Stories` roll-up refreshed · board mirror re-rendered when attached · knowledge-state
report delivered with Known-Unknowns owners, closing "sizing left to the team; the conversation
continues in the sprint."
...
- **Fast pass (score 1–2) adds:** happy path confirmed · rule-line checklist Confirmation · the
  knowledge-state report names the skipped mining loop and sweeps.
- **Score 4–5 adds:** Confirmation scoped to what IS known (clear hypothesis + first-probe AC) ·
  blocking questions verbatim in the Known-Unknowns block with owners · at most a recommendation
  that the team consider a timeboxed investigation.
...
(the skill compensates via hat rotation, it does not host a workshop) · two-way board sync (Spec A:
the mirror is strictly files → board) · refine-epic changes.

## 11. Test plan (added 2026-07-14 — required before the build ships)

Skill-authoring discipline: no skill ships on inspection alone — baseline first, then verify the
built skill against the same scripts.

- **Pressure personas** (scripted SME simulations, run as subagent conversations): the **rambler**
  (multi-topic answers — one thread, one question held?) · the **solution-jumper** (demands features
  and Gherkin early — chunk-up fires, Gherkin deferred to drafting?) · the **estimate-demander**
  ("just tell me how many points" — no-estimates lock holds while the capacity-bet stays
  elicitation-only?) · the **everything-is-mandatory** SME (does PARK still happen, or the hidden-
  waste flag fire?) · the **vague-word fountain** ("fast, seamless, robust" — every hit converted?).
- **Baseline (RED):** run each persona against a vanilla agent given only the store files; record
  verbatim where it stacks questions, drafts live Gherkin, invents numbers, or certifies readiness.
- **Compliance (GREEN):** the same scripts with the built skill; every §8 anti-pattern is a check.
- **Micro-tests** on load-bearing wording before full runs: the triage fork (fast lane taken when
  all risks are minor — no gold-plating; discovery exit taken on a major assumption-grade value
  risk) · the route-scoped §9 bars (a fast lane ends without padding) · Confirmation routing
  (values-differ → table; context/event-differs → GWT) · the vital-filter heuristic (a 2-criterion
  fast-pass story passes the gate).
- **Store round-trip:** materialize a fixture backlog → refine-feature full pass + fast lane +
  discovery exit → refine-story full + fast pass → verify statuses, IDs (unique, sequential,
  permanent), roll-up regeneration, and — when Miro tools are present — mirror re-render and the
  truncation marker pointing at the file.
