...
- **Wrap rule:** the tokens `build-story`, `building | built`, `Proof left to the Meters`, `no story, no code`, `#7c4dff`, and `#0e6b45` must never be split across lines in any new content (grep oracles depend on them).
...
**Files:**
- Create: `tests/fixtures/build-story/package.json`, `tsconfig.json`, `.gitignore`, `src/index.ts`, `src/smoke.test.ts` (plus `package-lock.json`, generated by Step 2's `npm install` and COMMITTED — it pins the fixture's dependency tree for reproducible campaign runs)
...
**Contamination caveat (record in both evidence docs):** the dispatch prompt names no skill, but subagents in this harness may still see ambient skill listings (e.g. superpowers TDD skills). That can only SUPPRESS predicted failures — never fake them — so observed failures remain valid counter-table material; note the caveat so readers know the baseline is conservative.

**Defined exit if the gate can't be met:** if after a THIRD run fewer than 2 distinct predicted failure modes have been observed, STOP — do not invent counter-table rows and do not keep burning runs. Report the three transcripts to Dan for a spec-deviation decision (the counter-table's observed-only rule vs acceptance criterion 1), and record the stop in the ledger.
...
- [ ] **Step 5: Verify and commit** — both docs exist, each with ≥1 verbatim quote or an explicit "silent" note per observed failure; acceptance criterion 1 and Task 5's counter-table need **≥2 DISTINCT predicted failure modes observed** across the two runs — two runs exhibiting only the same single mode do not pass this gate (if under 2 distinct, dispatch one more run before proceeding and say so in the ledger).
...
grep -c "#7c4dff" plugins/decompose-epic/skills/decompose-epic/references/backlog-store.md
grep -c "#0e6b45" plugins/decompose-epic/skills/decompose-epic/references/backlog-store.md
grep -rn "skeleton | ready | parked | superseded" plugins/
```

Expected: five `EXIT:0` lines; `1`; `4` (chain, holder list, table row, paragraph — the token is never wrapped); `1`; `1` (the theme-color edit is otherwise the one silently-skippable anchor — these two greps pin it); no output from the last grep (exit 1 = pass — the old schema comment is gone everywhere).
...
Expected: `1`
Run: `grep -c "references/backlog-store.md" plugins/build-story/skills/build-story/SKILL.md`
Expected: `1` (the store-contract pointer — without it the skill orphans the reference holding the theme colors and roll-up format its own phases need)
...
2. **Suite mentions** — old (exact, lines ~37–38):
...
Expected: `1`
Run: `grep -c "first of \*\*six\*\*" README.md`
Expected: `1`
Run: `grep -c "take each ready story to built" README.md`
Expected: `1`
Run: `grep -cF ".build-story/" README.md`
Expected: `2` (the state-dir mention + the gitignore instruction in the rule block)
...
- [ ] **Step 1: Stage a scratch copy** — `cp -r tests/fixtures/build-story <scratch>/green-run`, then in it: `npm install --no-audit --no-fund`, then `git init -q && git add -A && git commit -qm "fixture baseline"` — the scratch must be a real git repo so the skill's one-story-one-branch and commit-references-the-ID steps are exercised for real, not waved off.
...
```bash
git add docs/superpowers/evidence
git commit -m "docs: build-story GREEN evidence + hook realism replay + seeded negative test"
```
